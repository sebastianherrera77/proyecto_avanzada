...
with st.sidebar:
    ...
    menu_principal = st.radio(
        ...
        ["Inicio", "Gráficos", "Acerca", "Nosotros"],
        index=0
    )
...
if menu_principal == "Inicio":
    st.header("Bienvenido al Dashboard de Residuos")
    st.write("""
    En esta sección podrás visualizar información detallada sobre residuos municipales 
    a nivel nacional, por departamento, provincia y distrito.
    
    Selecciona **Gráficos** en el menú lateral para comenzar.
    """)
    st.image("logo.png", width=250)  # ← opcional, NO lo uses si no tienes logo

elif menu_principal == "Gráficos":
    st.header("📊 Visualización de Gráficos")

    # Sub-menú SOLO dentro de Gráficos
    opcion = st.radio(
...
elif menu_principal == "Acerca":
    st.header("Acerca del Proyecto")
    st.write("""
    Proyecto desarrollado para la visualización y análisis de residuos municipales 
    en Perú (2014 – 2021).
    """)
    ...
    st.header("Nosotros")
    st.write("""
    Proyecto académico – Ingeniería Ambiental.
    """)
